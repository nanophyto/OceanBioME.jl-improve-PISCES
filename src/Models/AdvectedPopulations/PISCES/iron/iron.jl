...
@inline ligand_concentration(iron::SimpleIron, inputs::IronInputs) = ligand_concentration(iron, inputs.DOC)
...
@inline free_iron(iron::SimpleIron, inputs::IronInputs) = free_iron(iron, inputs.Fe, inputs.DOC, inputs.T)
...
@inline function ligand_aggregation(
    iron::SimpleIron,
    scavenging_rate,
    Fe,
...
@inline function ligand_aggregation(iron::SimpleIron, inputs::IronInputs)
    return ligand_aggregation(
        iron,
        inputs.scavenging_rate,
        inputs.Fe,
        ligand_concentration(iron, inputs),
        free_iron(iron, inputs),
    )
end

@inline function iron_tendency(
    small_particles,
    grazing_waste,
    upper_trophic_waste,
    consumption,
    ...
    return (small_particles + grazing_waste + upper_trophic_waste - consumption -
            ligand_aggregation_loss - colloidal_aggregation - scavenging - bacterial_uptake)
end

@inline function iron_tendency(iron::SimpleIron, inputs::IronInputs)
    ligand_aggregation_loss = ligand_aggregation(iron, inputs)

    return iron_tendency(
